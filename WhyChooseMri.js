    ...
    return ( <
        ...
        div className = "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 " > {
            features.map((feature, index) => ( <
                div key = { index }
                className = "flex flex-col items-center bg-gray-800 p-6 rounded-lg shadow-lg transition-transform transform hover:scale-105 hover:shadow-2xl hover:bg-gray-700 duration-300 ease-in-out cursor-pointer" >
                <
                Image src = { feature.imgSrc }
                alt = { feature.title }
                className = "w-96 h-44 rounded-lg mb-4" / >
                <
                h3 className = "text-xl font-semibold" > { feature.title } < /h3> <
                p className = "text-gray-400 mt-2 text-center" > { feature.description } <
                /p> <
                /div>
            ))
        } <
