...
const FandQ = () => {
    ...
    return ( <
        ...
        <
        div className = "max-w-6xl mx-auto px-4" > { /* Title */ } <
        ...
        <
        Image src = "/images/Desease/bt4.jpg"
        alt = "Bone Tumor"
        width = { 500 }
        height = { 300 }
        ...
        div className = "bg-white p-6 rounded-lg shadow-md" > {
            faqs.map((faq, index) => ( <
                div key = { index }
                className = "mb-4" > { /* Question */ } <
                h2 className = "text-lg font-semibold flex justify-between items-center cursor-pointer"
                onClick = {
                    () => toggleVisibility(index)
                } > { faq.question } <
                button className = "text-gray-600" > { visibleIndex === index ? < HiChevronUp size = { 25 } /> : <HiChevronDown size={25} / > } <
                /button> < /
                h2 >
